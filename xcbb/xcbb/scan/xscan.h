...
private:
    ...
    __forceinline__ cudaError_t ReduceThenScanPass(ExclusiveScanStorage<Key>& storage);

public:

    explicit ExclusiveScanEnactor(int numElements);
    __forceinline__ cudaError_t Enact(ExclusiveScanStorage<Key>& storage);
};
...
template <typename Key>
__forceinline__
cudaError_t ExclusiveScanEnactor<Key>::Enact(ExclusiveScanStorage<Key>& storage)
{
...
template <int NUM_ELEMENTS_PER_THREAD, int NUM_WARPS>
__forceinline__
cudaError_t ExclusiveScanEnactor<Key>::ReduceThenScanPass(ExclusiveScanStorage<Key>& storage)
{
